...
The time it takes to charge the capactor to potential difference $\Delta V_\text{battery}$ depends on:
\begin{itemize}
...
\defitheorem{Capacitance}{
	...
	\begin{align*}
		...
		\Delta V&=\text{potential difference (V)}
	\end{align*}
	Capacitance is fixed by the geometry of the the capacitor. The unit for capacitance is the \textbf{farad} (\si{F}), which is defined as a coulombs per volt (\si{C/V}).
...
\definition{An \textbf{eqiuvalent capacitor} is a capacitor that has equal capacitance, total charge and potential difference as the original circuit.}
...
\begin{equation*}
    \sum Q_i=\sum Q_i'
\end{equation*}
Potential energy $U_c=Q\Delta V/2$ decreases during redistribution, as enregy is dissipated as thermal energy.
...
\defitheorem{Time constant}{
	The \textbf{time constant} ($\tau$) of an RC circuit is defined as the the product of resistance and capacitance:
	\begin{equation*}
	    \tau=RC
	\end{equation*}
	where:
