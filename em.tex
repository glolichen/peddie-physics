...
\remark{Electric bills}{
	...
	\begin{equation*}
		1\,\text{kWh}=3.6\cdot10^6\,\si{\joule}
	\end{equation*}
}
...
\end{equation*}
An electron has a charge of $-1.6\cdot 10^{-19}\,\si{\coulomb}$, and the charge of a proton has the same magnitude but opposite sign: $+1.6\cdot 10^{-19}\,\si{\coulomb}$.
...
\theorem{Constants (on equation sheet)}{
	...
	\begin{align*}
		m_e&=9.11\cdot 10^{-31}\,\si{\kilo\gram}
		...
		Q_e&=1.60\cdot 10^{-19}\,\si{\coulomb}
		\intertext{The gravitational constant from Mechanics is:}
		G&=6.67\cdot 10^{-11}\,\si{\newton\meter^2\per\kilo\gram^2}
	\end{align*}
	SI prefixes: $10^{-3}$ milli (m), $10^{-6}$ micro ($\mu$), $10^{-9}$ nano (n), $10^{-12}$ pico (p).
}

For two electrons spaced 1 millimeter apart, we can calculate $F_E=k \left|q_1\right|\left|q_2\right|/r^2$ and $F_G=Gm_1m_2/r^2$. It turns out that $F_E$ is about $4\cdot 10^{42}$ times greater than $F_G$. So the gravitational force can often be ignored because it is so weak compared to electric force.
...
\theorem{Gravitational potential}{
...
\theorem{Electric potential and potential energy, uniform field}{
	In a uniform electric field (between two charged plates) of strength $E$, the electric potential energy $U_E$ of a charge $q$ at a distance $x$ to the plate of opposite charge from $q$ is:
	\begin{equation*}
	    \Delta U_E=qEx
	\end{equation*}
...
\definition{\textbf{Equipotential lines/surfaces} are liens/surfaces on which electric potential is constant. They are described in terms of the electric potential on all points on the line/surface measured in volts (V). They behavior similarly to contours.}
