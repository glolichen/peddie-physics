...
\end{equation*}
This integral is evaluated from areas of low potential to areas of high potential. The capacitance is:
\begin{equation*}
    C
	=\frac{Q}{\Delta V}
...
\end{equation*}
The capacitance depends only on geometry and not on the charge on the plates $Q$. If the order of bounds of the integral for $\Delta V$ is switched, it is important to ensure $C$ is a positive value.
...
Outside the magnet, magnetic field lines point from the north to the south pole. There are magnetic field lines inside the material as well in such a way to make them closed loops (this means that \ul{inside} a bar magnet, the lines are from south to north). The symbol $B$ refers to a magnetic field. In a simplified field diagram:
\begin{itemize}
...
When a magnet is dropped inside a magnetic field, it will orient itself such that the magnetic field lines are aligned/pointing in the same direction.
...
\theorem{Direction and magnitude of magnetic field due to wires}{
	...
	where:
	\begin{align*}
		B&=\text{magnitude of magnetic field (T)} \\
...
\theorem{Magnetic force on current-carrying wire}{
	The force exerted on a current-carrying wire by a magnetic field is:
	\begin{equation*}
	    F_B=l\cdot \mathbf I\times \mathbf B
	\end{equation*}
	where:
	\begin{align*}
		F_B&=\text{magnetic force (N)} \\
		\mathbf I&=\text{current (A)} \\
		l&=\text{length of wire in magnetic field (m)} \\
		\mathbf B&=\text{magnetic field (T)}
	\end{align*}
	The direction of the force can be determined by the right hand rule. $\mathbf I$ is a vector pointing in the direction of conventional current, and length $l$ is a scalar.
}

\theorem{Force between two parallel wires}{
	The magnitude of force exerted by two parallel current-carrying wires on each other is:
	\begin{equation*}
	...
	\begin{align*}
		F_B&=\text{magnetic force (N)} \\
		I_1&=\text{current in wire 1 (A)} \\
		I_2&=\text{current in wire 2 (A)} \\
		l&=\text{length of wires (m)} \\
		r&=\text{distance between wires (m)} \\
		\mu_0&=\text{vacuum permeability (\si{Tm/A})} \\
			 &=4\pi\times 10^{-7}\,\si{Tm/A}
	\end{align*}
	Wires with parallel currents attract each other while wires with anti-parallel currents repel.
}

\theorem{Magnetic field at center of current-carrying coils}{
	The magnetic field at the center of a coil/loop is:
	\begin{equation*}
	    B=\frac{\mu_0I}{2r}
	\end{equation*}
	where:
	\begin{align*}
		B&=\text{magnetic field (T)} \\
		I&=\text{current in coil (A)} \\
		r&=\text{radius of coil (m)} \\
		\mu_0&=\text{vacuum permeability (\si{Tm/A})} \\
			 &=4\pi\times 10^{-7}\,\si{Tm/A}
	\end{align*}
}
...
\theorem{Solenoid}{
	A solenoid (multiple coils of wire) behaves as an electromagnet: a magnet where the north pole is the end where conventional current enters (``+''), and the south pole is where conventional current exits (``-''). The strength of the magnetic field is:
	\begin{equation*}
	    B=\mu_0 nI
	\end{equation*}
	where:
	\begin{align*}
		B&=\text{magnetic field (T)} \\
		n&=\text{number of coils per unit length (\si{m^{-1}})} \\
		I&=\text{current (A)} \\
		\mu_0&=\text{vacuum permeability (\si{Tm/A})} \\
			 &=4\pi\times 10^{-7}\,\si{Tm/A}
	\end{align*}
...
\begin{itemize}
	\item Generators and motors (mechanical energy $\longleftrightarrow$ electrical energy).
	\item Microphones and speakers (sound energy $\longleftrightarrow$ electrical energy).
	\item Transformers, rail guns and oven/cooktop/``induction stoves.''
\end{itemize}

\textbf{Potential difference induced by magnetic field.} When a neutral conductor is placed in a magnetic field, it is polarized as electrons are moved by $F_B=q\cdot \mathbf v\times\mathbf B$. The polarization leads to an electric force $F_E$ pointing in the \ul{opposite} direction to $F_B$. $F_E$ and $F_B$ will equilibrate.
\begin{equation*}
    F_E=F_B
	\implies \cancel q E=\cancel q\cdot\mathbf v\times\mathbf B
	\implies E=\mathbf v\times\mathbf B
\end{equation*}
The potential difference between the polarized ends of the conductor is:
\begin{equation*}
    \Delta V=Ed=l\cdot \mathbf v\times \mathbf B =Blv
\end{equation*}
where $\Delta V=\varepsilon$ is the potential difference established across conductor as a result of relative movement between a conductor and the magnetic field, and $l$ is the length of the conductor. $B$ is the magnitude (strength) of the electric field: $B=\lVert \mathbf B \rVert $. $v$ is the relative speed between the conductor and the magnetic field. The latest equality $\Delta V=Blv$ is only true when velocity $v$ and magnetic field $B$ are orthogonal: $\lVert \mathbf v\times\mathbf B\rVert=vB$.

Relative motion between a conductor and a magnetic field \ul{induces} a potential difference in the conductor.

\textbf{Rail system.} A rail system consists of a conductor, a magnetic field and two \textbf{rails}. The rails are attached to the conductor and joined by a resistor with resistance $R$. The rails are stationary and conductive.

The potential difference induced by the magnetic field creates a current through the rails and the resistor.

This system is a generator: the movement/mechanical energy of the conductor through the magnetic field induces a potential difference and generates an electric current.
\begin{equation*}
    I=\frac{\Delta V}{R}=\frac{Blv}{R}
\end{equation*}
The induced current flows in a direction that opposes the motion of the rod and magnetic force. The magnetic force on the current in the wire has magnitude $F_B=IlB$.

\theorem{Lenz's law}{
	Induced current always flows in such a way to oppose the change that caused it.
}

Recall from power $P=Fv$, the power of the applied force (force moving the conductor across the field) is:
\begin{equation*}
    P
	=F_Av
	=F_Bv
	...
	=\frac{B^2l^2v^2}{R}
\end{equation*}
By conservation of energy, applied force equals magnetic force: $F_A=F_B$. The power of the electrical current through the rails is:
\begin{equation*}
	P 
	=I^2R
...
\theorem{Magnetic flux}{
	\begin{equation*}
	    \Phi_B
		=\oint B\cdot \mathrm{d}A
		=BA\cos\theta
	\end{equation*}
	where:
	\begin{align*}
		\Phi_B&=\text{magnetic flux ($\si{Tm^2}=\si{Vs}$)} \\
		B&=\text{magnitude of magnetic field (T)} \\
		A&=\text{area (\si{m^2})} \\
		\theta&=\text{angle between area vector and magnetic field}
	\end{align*}
}

\textbf{Faraday's law.} Monitor the magnetic flux $\Phi_B$ inside the loop of a rail system.
\begin{equation*}
    \Phi_B
	=BA\cos\theta
	=BA
\end{equation*}
$A$ is the area of the current loop: through the rail, into the resistor, through the other rail, and back through the conductor. The directed area vector of this loop is parallel to the magnetic field, so $\theta=0$. As time increases, the area bound by the loop increases.

The change in flux with respect to time is:
\begin{equation*}
    \frac{\Delta \Phi_B}{\Delta t}
	=\frac{\Delta(Bls)}{\Delta t}
	=Bl\cdot \frac{\Delta s}{\Delta t}
	=Blv
	=\Delta V
\end{equation*}
where $s$ is the length/width of the loop (the distance between the resistor and the conductor).
\theorem{Faraday's law}{
	\begin{equation*}
		\Delta V=-N \cdot \diff{\Phi_B}{t}
	\end{equation*}
...
\theorem{Biot-Savart law}{
	The Biot-Savart law is used to find the magnetic field caused by a constant current.
	\begin{align*}
	    \mathrm{d}\vec B
		&=\frac{\mu_0}{4\pi}\cdot I\cdot \frac{\mathrm{d}\hat l\times \hat r}{r^2} \\
	    \mathrm{d}B
		&=\frac{\mu_0}{4\pi}\cdot I\cdot \frac{\mathrm{d}l\sin\theta}{r^2}
	\end{align*}
	where:
	\begin{align*}
		I&=\text{current (A)} \\
		\mathrm{d}l&=\text{small wire segment in the direction of current} \\
		\hat r&=\text{unit vector from wire segment to point of interest} \\
		r&=\text{distance from wire segment to point of interest (m)} \\
		\theta&=\text{angle between $\mathrm{d}l$ and $\hat r$} \\
		\mu_0
		&=\text{vacuum permeability (\si{Tm/A})} \\
		&=4\pi\times 10^{-7}\,\si{Tm/A}
	\end{align*}
}
...
An \textbf{inductor} is a ``little'' solenoid. When current flows, a magnetic field is established in the inductor. If the current is changed, the magnetic flux is changed as well.

When the switch is closed, by Faraday's law and Lenz's law, a potential difference is induced in the inductor that opposes the change (of closing the switch) and of the potential difference from the battery. This induced potential difference in the inductor is called ``self-induced'' or ``back emf/$\Delta V$''.

Inductors (temporarily) resist changes in current. At time $t=0$, the current is zero since the inductor acts like a battery with opposite polarity. As time $t\to\infty$, the current approaches as if there were no inductor: $I=\Delta V/R$, and the current is steady.

The graph of $\Delta V$ against time is increasing and concave down.

Energy is stored in the inductor but the inductor does not have any resistance.

In a solenoid, there is a magnetic field is in the same direction as current. The flux for a single loop is:
\begin{equation*}
    \Phi_B=BA\cos\theta=BA
\end{equation*}
If the inductor has $n$ loops, the flux is $n\Phi_B$. This is proportional to current $I$.
\begin{equation*}
    N\Phi_B=LI
\end{equation*}
$L$ is the \textit{inductance}.

\theorem{Inductance}{
	Inductance $L$ is the proportionality constant between the flux of the inductance and current.
	\begin{equation*}
	    n\Phi_B=LI 
		\qquad
		L
	...
	where:
	\begin{align*}
		n&=\text{number of loops in inductor} \\
		\Phi_B&=\text{magnetic flux (\si{T\cdot m^2})} \\
		B&=\text{magnetic field (B)} \\
		A&=\text{area (\si{m^2})} \\
		I&=\text{current (A)}
	\end{align*}
	The unit of inductance is the henry (H).
	\begin{equation*}
		\left[ \frac{\si{T\cdot m^2}}{\si{A}}=\si{H} \right]
	\end{equation*}
}
...
\begin{equation*}
    B=\mu_0 nI
\end{equation*}
So inductance equals:
\begin{equation*}
    L=\frac{nBA\cos\theta}{I}=\frac{\mu_0 n^2 A}{l}
\end{equation*}
where $n$ is the number of loops, $A$ is the cross-sectional area and $l$ is the length of the inductor.

\textbf{Induced potential difference and inductance.} By Faraday's law, the change in magnetic flux with respect to time is the potential difference $\Delta V$.
\begin{equation*}
    LI=n\Phi_B
	\implies L\cdot \frac{\Delta I}{\Delta t}=n \cdot \frac{\Delta \Phi_B}{\Delta t}
	\implies
	\Delta V_\text{induced}=-L\cdot \frac{\Delta I}{\Delta t}=-L\cdot \diff It
\end{equation*}
The negative sign indicates that the induced potential difference opposes the change in current. The faster the current changes, the greater the induced current is. After a long time, the inductor acts just like a wire since the change in current over time approaches zero.

\theorem{Energy stored in inductor}{
	...
	where:
	\begin{align*}
		U_L&=\text{magnetic potential energy (J)} \\
		L&=\text{inductance (H)} \\
		I&=\text{current (I)}
	\end{align*}
}
