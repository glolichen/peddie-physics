...
A spring is like a string in that the tension is constant throughout the spring. If we divide a spring into smaller springs, they will have the same restoring force, which means the smaller springs will have a higher spring constant.
...
There is a mass $M$ oscillating with amplitude $A$.
\begin{align*}
    \Sigma F
	&=ma \\
	-kx
...
Note that arc length $s=l\theta$ where $l$ is the length of the string. A mass is attached to the end of the string and allowed to oscillate. Acceleration is the second derivative of arc length.
\begin{align*}
    \Sigma F
	&=ma \\
	-mg\sin\theta
...
A flat physical object of mass $m$ is oscillating. Let $r$ be the distance from the axis of rotation to center of mass. Parallel axis theorem:
\begin{equation*}
	I'=I_\text{cm}+Mr^2
\end{equation*}
...
The motion of a torsional pendulum is analagous to that of a spring-mass system.
