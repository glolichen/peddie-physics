...
\title{\vspace*{-40pt}AP Physics C: Mechanics Notes}
\author{Jayden Li}
...
\remark{Center of mass, rotation and translation}{
	If a force is applied on a free object, it will only translate if the force is in line with the center of mass, and will rotate and translate otherwise.
}
...
If there is no friction, the object will not roll, and will only slide down the slope.
...
\begin{itemize}
	...
	\item \hlc{Increase the number of trials with constant experimental conditions.}
...
In general, linear momentum and angular momentum are both conserved. Kinetic energy is not always conserved.
...
\definition{
	...
	\textbf{Simple harmonic motion} is oscillatory motion where there is a restoring force that is directly proportional to the displacement of the oscillator.
}

In a spring, the storing force is $k\Delta x$, which is directly proportional to displacement.
...
A spring is like a string in that the tension is constant throughout the spring. If we divide a spring into smaller springs, they will have the same restoring force, which means the smaller springs will have a higher spring constant. Figure 7.1.
...
In parallel: two separate springs are both attached to a mass $m$. Using the same notation as before, obviously:
\begin{equation*}
	\left\{\begin{gathered}
		\Delta x_1=\Delta x_2=\Delta x_\text{equivalent} \\
		F_1=F_2=F_\text{equivalent}
	\end{gathered}\right\}
	\implies F_1\Delta x_1=F_2\Delta x_2=F_\text{equivalent}\Delta x_\text{equivalent}
	\implies k_1=k_2=k_\text{equivalent}
\end{equation*}
...
\theorem{Period of various systems}{
	\begin{alignat*}{2}
		...
		\text{Physical pendulum:}& \qquad T&&=2\pi \sqrt{\frac{I_\text{cm}}{rmg}} \\
	...
	\begin{align*}
		...
		I_\text{cm}&=\text{moment of inertia about the center of mass (physical pendulum)} \\
...
\end{itemize}
Total mechanical energy is not conserved when there is drag. Negative work is done by the friction and some energy is transferred to thermal energy.
...
\end{equation*}
is the acceleration due to gravity near the planet's surface: $g=GM/r^2$. At higher distances from the surface, the distance $r$ is greater, so acceleration due to gravity is lower.

The relationship between the gravitational force exerted on an object $F_\text{g}$ and its center of mass's distance to the planet's center of mass $r$ is in Figure 8.1. $F_\text{g}$ is highest on the surface of the planet. From $0$ to the surface, $F_\text{g}$ increases linearly; for higher values, $F_\text{g}$ decreases proportional to $1/r^2$.
...
\end{equation*}
The total energy of a object orbiting another, denoted $E_\text{T}$, is the sum of its kinetic and (gravitational) potential energies. The velocity of the orbiting object is $v=\sqrt{GM/r}$ (can be found with $F_\text{c}=mv^2/r=GMm/r^2=F_\text{g}$).
\begin{equation*}
	E_\text{T}
...
\theorem{Total energy of orbiting object}{
	The total energy $E_\text{T}$ is exactly half the gravitational potential energy.
	\begin{equation*}
		E_\text{T}
		=-\frac12 \frac{GMm}{r}
		=\frac12 U_\text{g}
