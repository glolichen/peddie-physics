...
\end{equation*}
As time increases, $v_\text{cm}$ increases and $\omega$ decreases due to kinetic friction. Torque due to kinetic friction reduces angular velocity (high angular velocity first, which then decreases):
\begin{equation*}
	\omega=\omega_0+\alpha=w_0-\left( \frac{\mu_\text{k}mgR}{I} \right)t
\end{equation*}
At a certain time, the object will be rolling without slipping where $v_\text{cm}=\omega R$.
