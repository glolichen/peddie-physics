...
\newcommand{\definition}[1]{\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,parbox=true] #1 \end{tcolorbox}}
...
\definition{An object's change in position is known as \textbf{displacement}. The displacement $\Delta x=x_f-x_0$, where $x_f$ is the final position, and $x_0$ is the initial position. Displacement has a direction as well as a magnitude.}
...
\definition{\textbf{Time} is the interval over which change occurs. \textbf{Elapsed time} is the difference between the starting time of a motion and the ending time. The elapsed time $\Delta t=t_f-t_0$.}
...
\definition{\textbf{Speed} is different from velocity in that speed is a scalar and has no direction. At any time, an object's \textbf{Instantaneous speed} is the magnitude of its instanteneous velocity.}

\definition{\textbf{Average speed} is the distance traveled divided by elapsed time. Distance travaled can be greater than displacement, so average speed can be greater than average velocity.}
...
Motions along perpendicular axes (such as verticle and horizontal) can be analyzed separately. Let the $y$-axis represent the vertical axis and the $x$-axis represent the horizontal axis.
...
\definition{A \textbf{force} is a vector, with a magnitude and a displacement.}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,parbox=true]
...
\end{tcolorbox}
In $\mathbf{F}_{\text{net}}=m \mathbf{a}$, mass is measured in kilograms (kg) and acceleration is measured in meters per second squared (\si{\meter\per\second\squared}). The SI unit of force is the \textbf{newton}:
\begin{equation*}
...
If air resistance is negligible, the net force acting on a falling object is \textbf{weight}. All objects fall with the same acceleration of $g$. The magnitude of weight is denoted $w$, and we do not need to consider weight as a vector because the direction is always downward.

$F_{\text{net}}$ is the net force acting upon a falling object. By definition, $w$ is the net force. By Newton's second law:
\begin{align*}
	F_{\text{net}}=
	w&=ma
	\intertext{We know accleration is the gravitational constant $g$.}
	\implies \Aboxed{w&=mg}
\end{align*}
...
Weight acts on objects at all times, and therefore must be conteracted to keep an object stationary (and prevent it from falling further).
...
Normal force is often denoted with the symbol $\mathbf{N}$, which is \underline{\textbf{not}} the same as newtons (N).
