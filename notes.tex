...
\subsection{1D Inelastic Collisiosn}
...
\begin{align*}
	...
	\intertext{By cosine substraction identity $\cos(a-b)=\cos a\cos b+\sin a\sin b$:}
...
\begin{itemize}
    \item $v_1'=0$: object 1 transfers all its momentum to object 2, causing object 1 to come to a stop. This occurs when there is a head-on collision between two balls of equal mass, as we have spcified.
...
\begin{equation*}
    \tau=r_\perp F
\end{equation*}}
Torque is measured in newton-meters (\si{\newton\cdot\meter}). In the formula for magnitude of torqur $\tau=rF\sin\theta$, radius $r$ is measured in meters, magnitude of force $F$ is measured in newtons, and $\sin\theta$ is a scalar.
...
\begin{equation*}
    \tau=\frac12 mr^2\alpha=I\alpha
\end{equation*}
where $m$ is the object's mass, $r$ is the radius/distance from where the force is applied to the pivot point, and $\alpha$ is angular acceleration. The quantity $I=mr^2/2$ is called the \textbf{moment of inertia}.}
...
\definition{A system is in \textbf{unstable equilibrium} if once displaceed, it experiences a net force or torque in teh same direction of displacement. It will accelerate away from the equilibrium position even for small displacements.}
...
\begin{align*}
	v&=v_0+at
	\intertext{We know $v=r\omega$. $a$ is tangential/linear accleration $a_\text{t}=r\alpha$.}
...
The magnitude of weight/force of gravity is $F=w=mg$, where $m$ is the mass of the object. Substituting this into Newnton's universal law of gravitation:
\begin{equation*}
    mg=G \frac{mM}{r^2}
	\implies 
    g=\frac{GM}{r^2}
